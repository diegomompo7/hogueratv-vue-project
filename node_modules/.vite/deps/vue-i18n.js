import { At as onBeforeMount, Ft as onMounted, Kn as ref, L as Text, M as Fragment, Mn as effectScope, Rn as isRef, U as computed, Yn as shallowRef, et as createVNode, gn as watch, nt as defineComponent, pt as h, ut as getCurrentInstance, xt as inject, zt as onUnmounted } from "./vue.runtime.esm-bundler-BvoXUmaf.js";
...
/** @internal */
var warnMessages$2 = { [CompileWarnCodes.USE_MODULO_SYNTAX]: `Use modulo before '{{0}}'.` };
...
function createTokenizer(source, options = {}) {
	...
	function readText(scnr) {
		...
		while (true) {
			...
			else if (ch === "%") if (isTextStart(scnr)) {
				buf += ch;
				scnr.next();
			} else break;
...
function createParser(options = {}) {
	...
	function emitWarn(tokenzer, code, start, offset, ...args) {
		...
		if (onWarn) onWarn(createCompileWarn(code, location ? createLocation(start, end) : null, args));
	}
...
function fallbackWithSimple(ctx, fallback, start) {
	return [...new Set([start, ...isArray(fallback) ? fallback : isObject$1(fallback) ? Object.keys(fallback) : isString$1(fallback) ? [fallback] : [start]])];
}
/**
...
function checkHtmlMessage(source, warnHtmlMessage) {
	if (warnHtmlMessage && detectHtmlTag(source)) warn(format$2(WARN_MESSAGE, { source }));
}
...
function translate(context, ...args) {
	...
	if (isString$1(format) && context.messageCompiler == null) {
...
function createComposer(options = {}, VueI18nLegacy) {
	...
	if (translateExistCompatible && true) warnOnce(getWarnMessage(I18nWarnCodes.NOTICE_DROP_TRANSLATE_EXIST_COMPATIBLE_FLAG));
...
function convertComposerOptions(options) {
	...
	if (options.formatter) warn(getWarnMessage(I18nWarnCodes.NOT_SUPPORTED_FORMATTER));
	if (options.preserveDirectiveContent) warn(getWarnMessage(I18nWarnCodes.NOT_SUPPORTED_PRESERVE_DIRECTIVE));
	let messages = options.messages;
	if (isPlainObject(options.sharedMessages)) {
		const sharedMessages = options.sharedMessages;
		messages = Object.keys(sharedMessages).reduce((messages, locale) => {
			assign$1(messages[locale] || (messages[locale] = {}), sharedMessages[locale]);
			return messages;
		}, messages || {});
	}
...
function createVueI18n(options = {}, VueI18nLegacy) {
	{
		...
		const vueI18n = {
			...
			t(...args) {
				...
				let list = null;
				let named = null;
				if (!isString$1(arg1)) throw createI18nError(I18nErrorCodes.INVALID_ARGUMENT);
				const key = arg1;
				if (isString$1(arg2)) options.locale = arg2;
				else if (isArray(arg2)) list = arg2;
				else if (isPlainObject(arg2)) named = arg2;
				if (isArray(arg3)) list = arg3;
				else if (isPlainObject(arg3)) named = arg3;
			...
			tc(...args) {
				...
				const key = arg1;
				if (isString$1(arg2)) options.locale = arg2;
				else if (isNumber(arg2)) options.plural = arg2;
				else if (isArray(arg2)) list = arg2;
				else if (isPlainObject(arg2)) named = arg2;
				if (isString$1(arg3)) options.locale = arg3;
...
var Translation = /* @__PURE__ */ defineComponent({
	...
	setup(props, context) {
		...
		return () => {
			...
			return h(isString$1(props.tag) || isObject$1(props.tag) ? props.tag : getFragmentableTag(), assignedAttrs, children);
		};
	}
});
...
function vTDirective(i18n) {
	const _process = (binding) => {
		...
		if (modifiers.preserve) warn(getWarnMessage(I18nWarnCodes.NOT_SUPPORTED_PRESERVE));
...
function apply(app, i18n, ...options) {
	...
	if (globalInstall && useI18nComponentName) warn(getWarnMessage(I18nWarnCodes.COMPONENT_NAME_LEGACY_COMPATIBLE, { name: Translation.name }));
	if (globalInstall) {
...
function editScope(payload, i18n) {
	...
	if (composer) {
		...
		if (field === "locale" && isString$1(payload.state.value)) composer.locale.value = payload.state.value;
		else if (field === "fallbackLocale" && (isString$1(payload.state.value) || isArray(payload.state.value) || isObject$1(payload.state.value))) composer.fallbackLocale.value = payload.state.value;
...
function createI18n(options = {}, VueI18nLegacy) {
	...
	if (__legacyMode && __allowComposition && true) warn(getWarnMessage(I18nWarnCodes.NOTICE_DROP_ALLOW_COMPOSITION));
...
function useI18n(options = {}) {
	...
	if (__VUE_I18N_LEGACY_API__) {
		if (i18n.mode === "legacy" && !options.__useComponent) {
			if (!i18n.allowComposition) throw createI18nError(I18nErrorCodes.NOT_AVAILABLE_IN_LEGACY_MODE);
...
* @VueI18nGeneral
*/
var castToVueI18n = /* @__NO_SIDE_EFFECTS__ */ (i18n) => {
	if (!(__VUE_I18N_BRIDGE__ in i18n)) throw createI18nError(I18nErrorCodes.NOT_COMPATIBLE_LEGACY_VUE_I18N);
	return i18n;
};
...
function useI18nForLegacy(instance, scope, root, options = {}) {
	...
	if (isLocalScope && instance.proxy && !(instance.proxy.$options.i18n || instance.proxy.$options.__i18n)) throw createI18nError(I18nErrorCodes.MUST_DEFINE_I18N_OPTION_IN_ALLOW_COMPOSITION);
	const _inheritLocale = isBoolean(options.inheritLocale) ? options.inheritLocale : !isString$1(options.locale);
	const _locale = ref(!isLocalScope || _inheritLocale ? root.locale.value : isString$1(options.locale) ? options.locale : DEFAULT_LOCALE);
	const _fallbackLocale = ref(!isLocalScope || _inheritLocale ? root.fallbackLocale.value : isString$1(options.fallbackLocale) || isArray(options.fallbackLocale) || isPlainObject(options.fallbackLocale) || options.fallbackLocale === false ? options.fallbackLocale : _locale.value);
	...
	const locale = computed({
		...
		set: (val) => {
			if (_composer.value) _composer.value.locale.value = val;
			_locale.value = val;
		}
	});
	const fallbackLocale = computed({
		...
		set: (val) => {
			if (_composer.value) _composer.value.fallbackLocale.value = val;
			_fallbackLocale.value = val;
		}
	});
	const messages = computed(() => {
		if (_composer.value) return _composer.value.messages.value;
		else return _messages.value;
	});
	...
	function setPostTranslationHandler(handler) {
		if (_composer.value) _composer.value.setPostTranslationHandler(handler);
	}
	function getMissingHandler() {
		return _composer.value ? _composer.value.getMissingHandler() : _missing;
	}
	function setMissingHandler(handler) {
		if (_composer.value) _composer.value.setMissingHandler(handler);
	}
	...
	function mergeLocaleMessage(locale, message) {
		if (_composer.value) _composer.value.mergeLocaleMessage(locale, message);
	}
	function getDateTimeFormat(locale) {
	...
	function mergeDateTimeFormat(locale, format) {
		if (_composer.value) _composer.value.mergeDateTimeFormat(locale, format);
	}
	function getNumberFormat(locale) {
	...
	function mergeNumberFormat(locale, format) {
		if (_composer.value) _composer.value.mergeNumberFormat(locale, format);
	}
	const wrapper = {
		get id() {
		...
		set inheritLocale(val) {
			if (_composer.value) _composer.value.inheritLocale = val;
		},
		get availableLocales() {
		...
		set missingWarn(val) {
			if (_composer.value) _composer.value.missingWarn = val;
		},
		get fallbackWarn() {
		...
		set fallbackRoot(val) {
			if (_composer.value) _composer.value.fallbackRoot = val;
		},
		get fallbackFormat() {
			return _composer.value ? _composer.value.fallbackFormat : _fallbackFormat;
		},
		set fallbackFormat(val) {
			if (_composer.value) _composer.value.fallbackFormat = val;
		},
		get warnHtmlMessage() {
			return _composer.value ? _composer.value.warnHtmlMessage : _warnHtmlMessage;
		},
		set warnHtmlMessage(val) {
			if (_composer.value) _composer.value.warnHtmlMessage = val;
		},
		get escapeParameter() {
			return _composer.value ? _composer.value.escapeParameter : _escapeParameter;
		},
		set escapeParameter(val) {
			if (_composer.value) _composer.value.escapeParameter = val;
		},
		t,
		getPostTranslationHandler,
		setPostTranslationHandler,
	...
	onBeforeMount(() => {
		...
		} else if (isLocalScope) sync(composer);
	});
	return wrapper;
}
